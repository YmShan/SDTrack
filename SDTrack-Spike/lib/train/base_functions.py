...
from lib.train.dataset.coesot import coesot
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["EOTB_Train", "VISEVENT", 'COESOT']
        if name == "EOTB_Train":
            datasets.append(EOTB(settings.env.eotb_dir_train,split='train', image_loader=image_loader))
        if name == "VISEVENT":
            datasets.append(visevent(settings.env.visevent_train,split='train', image_loader=image_loader))
        if name == "COESOT":
            datasets.append(coesot(settings.env.coesot_train,split='train', image_loader=image_loader))
    return datasets
