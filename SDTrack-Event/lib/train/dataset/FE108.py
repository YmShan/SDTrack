...
class EOTB(BaseVideoDataset):
    def __init__(self, root=None, image_loader=opencv_loader, split=None):
        ...
        super().__init__('EOTB', root, image_loader)  # 调用父类的初始化函数
        ...
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'eotb_train_split_skx.txt')
            elif split == 'val':
                file_path = os.path.join(ltr_path, 'data_specs', 'eotb_val_split_skx.txt')
            else:
    ...
    def _get_event(self, seq_path, frame_id):
        ...
        event1 = os.path.join(seq_path, 'inter1_stack_3008', '{:04}_1.png'.format(frame_id + 1))
